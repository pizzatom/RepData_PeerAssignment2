...
Next, we note that the values in `PROPDMG` and `CROPDMG` need to be adjusted by orders of magnitude according the entries in `PROPDMPEXP` and `CROPDMPEXP` repsectively. The valid values are “K” for thousands, “M” for millions, and “B” for billions. We use a similar technique as we used to fix the `EVTYPE` variable to scale `PROPDMG` and `CROPDMG`. 
