...
```{r}
# NOTE TO SELF: consider using dplyr::group_by(EVTYPE) with dplyr::mutate and agrepl  
...
```{r group}
test_df <- df %>% 
           mutate(GRPS = sapply(EVTYPE$NAMES, agrepl, x=df$EVTYPE, ignore.case=TRUE, max.distance = 10)) 

```

```{r group}
plot_test <- test_df %>%
            select(EVTYPE,GRPS,INJURIES,FATALITIES) %>%
            mutate(HEALTH_IMPACT=INJURIES+FATALITIES) %>%
            group_by(GRPS) %>%
            summarize(TOTAL_HEALTH=sum(HEALTH_IMPACT))

g.test <- ggplot(plot_test, aes(x=GRPS, y=TOTAL_HEALTH))
g.test + geom_point()
```
...
A great way to understand this data is with a plot of all of the impactful event types. You can see in the graph that the tornados are far and away the most impactful weather event on human health. 
...
We measure economic consequences as the sum of property damage (`PROPDMG`), crop damage (`CROPDMG`). It's important to note the expression (`*EXP`) for these two variables, which provide the order of magnitude of the value. The valid values are “K” for thousands, “M” for millions, and “B” for billions.
