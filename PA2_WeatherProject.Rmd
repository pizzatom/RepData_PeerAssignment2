...
Looking at these unique entries also raises some questions about the cleanliness of this data. For example, `TSTM WIND` represents the same thing as `THUNDERSTORM WIND` which is the same as `THUNDERSTORM WINDS`, etc. To make matters more complicated, these examples (and others) represent three categories: `THUNDERSTORM` and `WIND` and `THUNDERSTORM + WIND`. IN summary, our data is not clean and not tidy, so we have some work to do. 
